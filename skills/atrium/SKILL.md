...
# If the card's adapter is limited when it launches, run on these instead.
...
- **Fallback when limited**: `--fallback <adapter>[:<profile>]`, repeated in order (max 8), replaces the card's fallback list; `--fallback-accounts-only` turns it on with no other providers; `--clear-fallback` removes it. None of these need `--adapter`. It applies to every launch of the card (scheduled fires, run-now, `task launch`, the in-app Launch button): before the run starts, if the card's adapter is rate/usage-limited, atrium first tries that adapter's other accounts, then each fallback in order. Unknown limit data never counts as limited, and a run never switches mid-way. When everything is limited, a scheduled run waits for the earliest reset (a card comment says so), `task launch` / run-now fail with a limited error naming it, and the in-app Launch button offers to launch the primary anyway. The run's `launchProfile.launchFallback` records what it skipped and why.
