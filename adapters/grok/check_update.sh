...
command -v grok >/dev/null 2>&1 || json_error "grok not found"

installed_output="$(GROK_DISABLE_AUTOUPDATER=1 grok --version 2>&1)" || json_error "failed to determine installed Grok version"
installed_version="$(extract_version "$installed_output")" || true
[[ -n "$installed_version" ]] || json_error "failed to parse installed Grok version"

registry_json="$(curl -fsS --connect-timeout 2 --max-time 5 'https://registry.npmjs.org/-/package/@xai-official%2Fgrok/dist-tags' 2>/dev/null)" || json_error "failed to fetch latest Grok version"
latest_version="$(printf '%s' "$registry_json" | jq -er '.latest | select(type == "string" and length > 0)' 2>/dev/null)" || json_error "failed to parse latest Grok version"
[[ "$latest_version" =~ ^[0-9]+\.[0-9]+\.[0-9]+([+-][0-9A-Za-z.-]+)?$ ]] || json_error "failed to parse latest Grok version"
