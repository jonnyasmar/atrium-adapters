// atrium.ts — omp extension that forwards session, agent, and tool
// lifecycle events to atrium's `hook emit` interface. Installed by
// adapters/omp/hooks.sh to ~/.omp/agent/extensions/atrium.ts.
//
// Omp auto-discovers TS extensions in ~/.omp/agent/extensions/ and compiles
...
const INPUT_REQUEST_TOOLS_ENV = "ATRIUM_INPUT_REQUEST_TOOLS_OMP";
const DEFAULT_INPUT_REQUEST_TOOLS = ["ask"];

const DEBUG_LOG =
  process.env.ATRIUM_OMP_EXTENSION_LOG || join(tmpdir(), "atrium-omp-extension.log");
const DEBUG = process.env.ATRIUM_OMP_EXTENSION_DEBUG !== "0";
...
debug("atrium omp extension loaded", { ATRIUM_ACTIVE, ATRIUM_CLI, ATRIUM_PANE_ID });
...
// Derive a stable session id from the omp session file path. Omp stores
// sessions as `~/.omp/agent/sessions/<cwd-encoded>/<ISO-timestamp>_<uuid>.jsonl`;
// the uuid after the last underscore is the session id (matching the id on the
// file's `session` header line and what list_recent_sessions.sh returns, so
// live activity cards correlate with resumable session cards). For brand-new
// ephemeral sessions (file not written yet), fall back to the pane id so
// atrium can still create the activity card.
function resolveSessionId(ctx: { sessionManager?: { getSessionFile?: () => string | null } }): string {
  ...
  return ATRIUM_PANE_ID || "omp-ephemeral";
}
