...
[{
  ...
  "hooks": [{
    "type": "command",
    "command": "PORT=$(cat ~/.atrium/hook-port 2>/dev/null) && [ -n \"$PORT\" ] && curl -s -X POST http://127.0.0.1:$PORT/api/adapter/claude-code/session-start -H 'Content-Type: application/json' -d \"$(cat)\"",
    "timeout": 5
  }]
}]
HOOKJSON
...
[{
  ...
  "hooks": [{
    "type": "command",
    "command": "PORT=$(cat ~/.atrium/hook-port 2>/dev/null) && [ -n \"$PORT\" ] && curl -s -X POST http://127.0.0.1:$PORT/api/adapter/claude-code/session-end -H 'Content-Type: application/json' -d \"$(cat)\"",
    "timeout": 5
  }]
}]
HOOKJSON
